...
#[derive(Debug, Clone, Copy, Default)]
pub(crate) enum HighlightStatsModel {
    #[default]
    Off,
    Clip {
        affected_sites: usize,
        changed_sites: usize,
        nominal_over_white_sites: usize,
        affected_by_channel: [usize; 3],
    },
    LocalRatios {
...
pub(crate) fn show(
    ...
    egui::Window::new("Developer diagnostics")
        ...
        .show(context, |ui| {
            ...
            match preview.highlight {
                HighlightStatsModel::Off => {
                    ui.label("Highlight reconstruction: off");
                }
                HighlightStatsModel::Clip {
                    affected_sites,
                    changed_sites,
                    ...
                    ui.label(format!(
                        "Highlight Clip: {} affected · {} changed · {} nominally over-white",
                        affected_sites, changed_sites, nominal_over_white_sites,
                    ));
                    ui.label(format!(
                        "Affected CFA sites: R {} · G {} · B {}",
                        affected_by_channel[0], affected_by_channel[1], affected_by_channel[2],
                    ));
                }
...
pub(crate) enum HighlightReport {
    Off,
    Clip {
        affected_sites: usize,
        changed_sites: usize,
...
impl From<HighlightStatsModel> for HighlightReport {
    fn from(value: HighlightStatsModel) -> Self {
        match value {
            HighlightStatsModel::Off => Self::Off,
            HighlightStatsModel::Clip {
                affected_sites,
                changed_sites,
                nominal_over_white_sites,
