...
| 4 | Move optics and preview resampling to GPU | Resident capture, optics, exact area reduction, preview/Source 1:1, and integer-band export are implemented. Software deterministic parity is exercised; aggregate memory/lifecycle hardening, real-image visual review, and hardware/interaction qualification remain open. See the [implementation plan and evidence](gpu-optics-and-preview-resampling.md). |
...
The [milestone 4 implementation plan](gpu-optics-and-preview-resampling.md)
covers the resident capture-completed source, shared optics execution contract,
exact banded area reduction, removal of the CPU optics bridge, and preview,
Source 1:1, and export qualification gates.
